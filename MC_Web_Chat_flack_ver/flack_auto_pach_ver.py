...
def get_log_file_path(version=None):
    ...
    if version:
        ...
        log_path = os.path.join(version, 'logs', 'latest.log')
...
def get_minecraft_version_jar_path():
    for proc in psutil.process_iter(['pid', 'name']):
        if proc.info['name'] == 'javaw.exe':
            cmdline = proc.cmdline()
            if '--gameDir' in cmdline:
                jar_index = cmdline.index('--gameDir')
                jar_path = os.path.abspath(cmdline[jar_index + 1])
                return jar_path
    return None

def get_minecraft_DIR_path():
    for proc in psutil.process_iter(['pid', 'name']):
        if proc.info['name'] == 'javaw.exe':
            cmdline = proc.cmdline()
            if '--gameDir' in cmdline:
                jar_index = cmdline.index('--gameDir')
                jar_path = os.path.abspath(cmdline[jar_index + 1])
                minecraft_dir_path = os.path.join(jar_path, '..', '..')
...
if __name__ == '__main__':
    ...
    if version_isolated == '是':
        version = get_minecraft_version_jar_path()
    else:
        version = get_minecraft_DIR_path()

    start_log_monitoring(version)

    threading.Thread(target=socketio.run, args=(app, ), kwargs={'host': '0.0.0.0', 'port': 8080,'allow_unsafe_werkzeug':True}).start()
